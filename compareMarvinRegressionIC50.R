...
plotIfnsMarvin<-function(dilutes,concAlpha,xlab='',marvins){
  ...
  for(thisSample in unique(dilutes$sample)){
    ...
    text(10^par('usr')[1]*2,10^par('usr')[3]*1.3,sprintf('Marvin Vres=%s IC50=%s',format(marvins[thisSample,'vres'],digits=2),format(marvins[thisSample,'ic50'],digits=2)),adj=0)
    plotIfn(concAlpha,xx,thisSample,xlab=xlab,log='x',scaleMax=TRUE)
    abline(v=marvins[thisSample,'ic50'],col='red',lty=2)
