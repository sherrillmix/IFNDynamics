...
for(ifn in c('ic50.IFNa2','ic50.IFNb')){
  plot(ifelse(manOut$day==0,patZeroPos[manOut$pat],manOut$day),manOut[,ifn],ylim=range(manOut[,ifn]),xlim=range(manOut$day[!is.na(manOut$day)]),log='y',yaxt='n',ylab=sprintf('%s IC50',sub('ic50\\.','',ifn)),pch=21,xlab='Days after detectable rebound',bg=patCols[manOut$pat],cex=1.2,mgp=c(2.3,.8,0))
  dnar::logAxis(las=1)
  if(ifn=='ic50.IFNb')legend('topright',names(patCols),pch=21,pt.bg=patCols,bty='y',inset=.01,y.intersp=.9)
}
dev.off()
