...
plot(ic50df$day,ic50df$ic50,log='xy',yaxt='n',pch=21,bg=cols[ic50df$patient],cex=1.4,col=cols[ic50df$patient],xaxt='n',xlab='Days after infection',ylab='IFNa2 IC50',mgp=c(2.5,.8,0),xlim=range(c(ic50df$day),2048))
logAxis(2,las=1)
axis(1,2^(1:12))
...
for(ii in unique(patient)){
  ...
  plot(ic50df$day,ic50df$ic50,log='xy',yaxt='n',xaxt='n',xlab='Days after infection',ylab='IFNa2 IC50',mgp=c(2.5,.8,0),type='n',main=sprintf('%s r^2=%0.3f',ii,summary(fit)$adj.r.squared),xlim=range(c(ic50df$day),2048))
  logAxis(2,las=1)
  axis(1,2^(1:12))
  fakeDays<-2^2:2^12
