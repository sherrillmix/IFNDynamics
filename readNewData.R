...
dat$sample<-sub('[._][^._]*$','',sub('_Bulk','BULK',sub('_BULK','BULK',sub('\\.([0-9]+)_Bulk','.\\1.BULK',dat$ID.for.Publications))))
...
pdf('out/time_vs_all.pdf',width=8,height=8)
  ...
  for(ii in unique(dat$pat)){
    withAs(xx=dat[dat$pat==ii,],plot(xx$time/7,xx$ic50,bg=patCols[xx$pat],pch=21,las=1,log='y',yaxt='n',xlab='',ylab='IFNa IC50',main=ii,xlim=range(dat$time/7),ylim=range(dat$ic50,na.rm=TRUE)))
    title(xlab='Time (weeks)',mgp=c(2,1,0))
    logAxis(las=1)
    thisDat<-dat[dat$pat==ii,]
    par(new=TRUE)
    withAs(xx=unique(dat[dat$pat==ii,c('time','vl')]),plot(xx$time/7,xx$vl,type='l',log='y',yaxt='n',xlab='',ylab='',xlim=range(dat$time/7),ylim=range(dat$vl,na.rm=TRUE),xaxt='n'))
    logAxis(4,las=1)
    text(convertLineToUser(3,4),10^mean(par('usr')[3:4]),'Viral load',srt=-90,xpd=NA)
    par(new=TRUE)
    withAs(xx=unique(dat[dat$pat==ii,c('time','CD4')]),plot(xx$time/7,xx$CD4,type='l',log='',yaxt='n',xlab='',ylab='',xlim=range(dat$time/7),ylim=range(dat$CD4,na.rm=TRUE),xaxt='n',col='red'))
