...
alphaNadirs<-unique(dat[dat$isNadir,c('pat','time')])
rownames(alphaNadirs)<-alphaNadirs$pat
betaNadirs<-unique(dat[dat$isBetaNadir,c('pat','time')])
rownames(betaNadirs)<-betaNadirs$pat
dat[dat$pat=='WEAU','isLast']<-FALSE
dat[dat$pat=='WEAU'&dat$timeBeforeArt==0,'isLast']<-TRUE
#acuteChronicMM<-dat[dat$qvoa|(dat$time<acuteCut|dat$time>=chronicCut),]
acuteChronicMM<-dat[dat$qvoa|(dat$time<acuteCut|(dat$time>=chronicCut&(dat$timeBeforeArt>=0|is.na(dat$timeBeforeArt)))),]
...
acuteChronicMM$displayClassAlpha<-ifelse(acuteChronicMM$qvoa,'Outgrowth',ifelse(acuteChronicMM$time<=acuteCut,'Acute',
    ifelse(acuteChronicMM$time<alphaNadirs[acuteChronicMM$pat,'time'],'Early',
    ifelse(acuteChronicMM$time==alphaNadirs[acuteChronicMM$pat,'time'],'Nadir',
    ifelse(!is.na(acuteChronicMM$timeBeforeArt)&acuteChronicMM$isLast,'Last','Late'
)))))
acuteChronicMM$displayClassBeta<-ifelse(acuteChronicMM$qvoa,'Outgrowth',ifelse(acuteChronicMM$time<=acuteCut,'Acute',
    ifelse(acuteChronicMM$time<betaNadirs[acuteChronicMM$pat,'time'],'Early',
    ifelse(acuteChronicMM$time==betaNadirs[acuteChronicMM$pat,'time'],'Nadir',
    ifelse(!is.na(acuteChronicMM$timeBeforeArt)&acuteChronicMM$isLast,'Last','Late'
)))))
acuteChronicMM$virus<-acuteChronicMM$id
...
rebound2$displayClass<-rebound2$class<-rebound2$displayClassAlpha<-rebound2$displayClassBeta<-ifelse(rebound2$Type1=='Rebound','Rebound',ifelse(grepl('Post',rebound2$Type1),'Post-ATI',ifelse(grepl('Pre|week',rebound2$Type1),'Pre-ATI','Outgrowth')))
...
write.csv(rebound2,'out/reboundVoaData.csv',row.names=FALSE)
...
rebound$displayClass<-rebound$displayClassAlpha<-rebound$displayClassBeta<-rebound$class<-ifelse(rebound$Type=='Rebound','Rebound',ifelse(grepl('Post',rebound$Type),'Post-ATI',ifelse(grepl('Pre',rebound$Type),'Pre-ATI','Outgrowth')))
...
pair$displayClass<-pair$displayClassAlpha<-pair$displayClassBeta<-sprintf('%s %ss',ifelse(pair$class=='Donor','Chronic','Acute'),pair$class)
...
targetCols<-c('pat','class','study','ic50_IFNa2','ic50_IFNb','displayClass','displayClassAlpha','displayClassBeta','virus','repCap','infectivity','p24Release')
...
combined$labelAlpha<-ifelse(combined$simplePat=='',combined$displayClassAlpha,sprintf('%s%s%s',combined$displayClass,ifelse(combined$simplePat!='',' ',''),combined$simplePat))
combined$labelBeta<-ifelse(combined$simplePat=='',combined$displayClassBeta,sprintf('%s%s%s',combined$displayClass,ifelse(combined$simplePat!='',' ',''),combined$simplePat))
