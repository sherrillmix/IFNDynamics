...
for(ii in sort(unique(dat$pat))){
  ...
  for(jj in names(virusCols)){
    plot(thisDat$time,thisDat[,virusCols[jj]],log='y',xaxt='n',ylim=range(dat[,virusCols[jj]],na.rm=TRUE),las=1,cex.axis=.5,ylab=sub(' ','\n',jj),cex.lab=.9,xlab='',yaxt='n',xlim=range(comboMeta$time[!comboMeta$qvoa]),cex.lab=2,mgp=c(2.5,.5,0),cex=1.5,pch=21,bg='#00000033')
    means<-10^tapply(log10(thisDat[,virusCols[jj]]),thisDat$time,mean,na.rm=TRUE)
    hasMean<-!is.nan(means)
    lines(as.numeric(names(means)[hasMean]),means[hasMean],lwd=2)
    logAxis(2,las=1,cex.axis=.6)
    text(mean(par('usr')[1:2]),10^(par('usr')[4]-.1*diff(par('usr')[3:4])),jj,col='#00000033',xpd=NA,adj=c(.5,1),cex=2.5)
    if(jj==names(virusCols)[1])text(sum(par('usr')[1:2]*c(.02,.98)),10^(par('usr')[4]-.1*diff(par('usr')[3:4])),ii,col='#00000099',xpd=NA,adj=c(1,1),cex=3.5)
    axis(1,xlabs*7,rep('',length(xlabs)))
  }
  for(jj in names(metaCols)){
    plot(rep(thisMeta$time,length(metaCols[[jj]])),unlist(thisMeta[,metaCols[[jj]],drop=FALSE]),ylim=range(comboMeta[,metaCols[[jj]]],na.rm=TRUE),las=1,cex.axis=.6,ylab=sub(' ','\n',jj),cex.lab=.9,xlab='',xaxt='n',xlim=range(comboMeta$time[!comboMeta$qvoa]),cex.lab=2,mgp=c(2.5,.5,0),pch=21,bg='#00000033',cex=1.5)
    means<-apply(thisMeta[,metaCols[[jj]],drop=FALSE],1,mean,na.rm=TRUE)
    hasMean<-!is.nan(means)
    if(any(hasMean))lines(thisMeta$time[hasMean],means[hasMean],lwd=2)
    text(mean(par('usr')[1:2]),(par('usr')[4]-.1*diff(par('usr')[3:4])),jj,col='#00000033',xpd=NA,adj=c(.5,1),cex=2.5)
    axis(1,xlabs*7,rep('',length(xlabs)))
  }
  axis(1,xlabs*7,xlabs,cex.axis=2)
...
pdf(sprintf('out/virus_vs_meta%s.pdf',ifelse(dayCut==0,'',sprintf('_after%ddays',dayCut))),height=8,width=8)
  for(jj in names(metaCols)){
    for(ii in names(virusCols)){
      ...
      plot(1,1,type='n',xlim=xlim,ylim=ylim,xlab=jj,ylab=ii,log='yx',yaxt='n',xaxt='n',cex.lab=1.5,mgp=c(2.8,1,0))
      logAxis(2,las=1)
      ...
      for(pat in sort(unique(dat$pat))){
        ...
        points(mMeans[isGood],vMeans[isGood],pch=21,bg=sprintf('%s99',patCols[pat]),cex=2)
        legend(convertLineToUser(2,4),convertLineToUser(1.5,1),names(patCols),pt.bg=patCols,pch=21,inset=c(.01,-.01),ncol=3,xpd=NA,xjust=1,bty='n',pt.cex=2)
