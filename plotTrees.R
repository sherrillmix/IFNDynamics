...
#,layout='unrooted'
print(ggtree(tree)+geom_tippoint(color=timeCols[times],size=2.5)+ggtitle(unique(subject))+theme(plot.title = element_text(hjust = .5,size=28,margin=margin(b=-10,unit='pt')))+geom_treescale(offset=-5,fontsize=5,x=.01,y=-8))
}
dev.off()
